...
当前处于 `Phase 1 Source Vault key bootstrap coordination implemented; object migration next`；`Phase 1 host acceptance complete` 是已有合成宿主验收事实，具体能力与近期缺口见[当前状态](docs/status/current.md)。
...
- P1-S02 选择 XChaCha20-Poly1305 + STREAM-BE32；P1-S03a 已完成 portable manifest / `Cargo.lock`、cipher / wrap / AAD 与合成测试。[P1-S03a 落地记录](docs/implementation/phase1-source-vault-portable-crypto.md)记录扩大到 344 项的 notices；[P1-S03b](docs/implementation/phase1-source-vault-filesystem.md)已实现 versioned envelope、immutable object filesystem 与认证回读，通过 macOS 合成测试及 Windows ARM64 / NTFS 提升权限、普通用户与 ACL 验收，文件身份替换缺陷已修复；Linux ARM64 / ext4 普通用户验收也已通过；[P1-S03c-2](docs/implementation/phase1-source-vault-key-provider.md) 的独立 platform provider 已实现；真实密钥库、SQLite migration 与宿主加密数据流尚未验收。

- [P1-S04a](docs/implementation/phase1-source-vault-key-bootstrap.md) 已实现 SQLite 密钥初始化事务、v7 维护 checkpoint 和合成失败 / 并发验证；普通入口仍停在 v6，下一步是正文对象迁移与恢复。Windows / Linux 验证按当前状态后置集中执行。
