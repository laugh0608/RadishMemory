...
- P1-S02 选择 XChaCha20-Poly1305 + STREAM-BE32；P1-S03a 已完成 portable manifest / `Cargo.lock`、cipher / wrap / AAD 与合成测试。[P1-S03a 落地记录](docs/implementation/phase1-source-vault-portable-crypto.md)记录扩大到 344 项的 notices；[P1-S03b](docs/implementation/phase1-source-vault-filesystem.md)已实现 versioned envelope、immutable object filesystem 与认证回读，通过 macOS 合成测试及 Windows ARM64 提升权限基线；Windows 普通用户 / 专属边界与 Linux 文件系统运行证据待补，三个 platform provider、SQLite migration 与宿主加密数据流尚未实现。
