...
范围：`M0-I02` canonical core 第一评审单元、workspace 工具链与聚合检查入口。
...
- 本地 macOS 已使用 Rust / Cargo `1.96.0` 运行全 workspace 的格式、Clippy 和全部 target 测试；core 的冻结 digest vectors、完整 suite digest 与补充的时间、NFC、JSON 和错误负例均通过；
...
本基线证明 canonical core 第一评审单元与当前依赖图在本机成立，不证明九种 canonical 顶层对象、跨对象引用、SQLite、FTS5、fixture runner 或任何完整产品闭环已经实现。
