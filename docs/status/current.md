...
下一实施单元进入 `M0-I03 SQLite entry`：先解析和审查 `rusqlite` bundled 依赖及许可证 / native build 影响，再建立最小连接、migration、FTS5 capability probe 与实际需要的 core port。依赖或 lockfile 变更必须与审查证据同一提交；不得用内存 fallback 掩盖 SQLite / FTS5 初始化失败，也不提前把 fixture operation 编进 adapter。
...
- 当前仓库检查证明治理、文本、链接、配置合同、canonical core primitive、九种对象、字段级校验和跨对象不变量的格式、lint、测试与 frozen digest vectors 在本机成立；不证明 SQLite / FTS5、runner、真实删除执行或同步已经实现，也不等同于三平台 CI 已运行。
...
- 待完成：SQLite / FTS5 adapter 和 M0 runner 真实实现；
