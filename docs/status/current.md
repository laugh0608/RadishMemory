...
`M0-I02` 的第一个独立评审单元已实现稳定 core 错误、RFC 3339 / UTC 时间、`ValidTime`、`exact-bytes-v1` / `utf8-nfc-text-v1` / `canonical-json-v1` SHA-256，以及拒绝重复 key 和 `null` 的 `radishmemory-canonical-json-v1` parser / writer。`M0-I02` 的第二个独立评审单元已实现九种 canonical 顶层对象、共同值类型与字段级条件校验；外部 `schema_version` / `object_type` mapping、同 namespace 引用解析、事件链和其它跨对象不变量仍未实现。当前 lockfile 包含四个经审查的 core 直接依赖和 29 个第三方 package，没有 Git dependency、产品网络能力或 native C / C++ 代码；build script、proc macro、feature 与许可证影响见 [M0 Rust 依赖基线](../implementation/m0-rust-dependency-baseline.md)。`rusqlite`、bundled SQLite 与 FTS5 尚未进入实现。

## 当前顺位

1. `M0-I02 canonical invariants`：在已通过的 primitive 与 object 单元上实现同 namespace 引用解析、proposal / decision / record / event 闭环、来源切片复验、状态投影和删除计划 / 证据集合一致性；不接数据库或 CLI 业务捷径。
...
`M0-I02` 前两个评审单元保持在 `radishmemory-core`、直接测试、依赖记录和必要检查器内：

1. 四个直接依赖与全部传递依赖已经 lock，并记录解析版本、许可证、feature、build script / proc macro、网络与 native code 影响；
...
下一实施单元仍属于 `M0-I02`：实现同 namespace 引用解析、来源正文切片复验、proposal / decision / record / initial event 闭环、事件链 / 当前状态投影与删除计划 / 证据集合等跨对象不变量。该单元未完成前不启动 SQLite、migration、FTS5 或 runner；外部 JSON mapping 仍须显式拒绝未知 schema / enum，不得把对象构造器当成兼容性 fallback。
...
- 当前仓库检查证明治理、文本、链接、配置合同、canonical core primitive、九种对象与字段级校验的格式、lint、测试和 frozen digest vectors 在本机成立；不证明跨对象不变量、SQLite / FTS5、runner、删除执行或同步已经实现，也不等同于三平台 CI 已运行。
...
- 待完成：canonical 跨对象不变量、SQLite / FTS5 adapter 和 M0 runner 真实实现；
