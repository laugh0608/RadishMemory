...
[ADR 0008](adr/0008-phase1-encrypted-source-vault.md) 已接受受管原始对象的本地认证加密契约，[P1-S02 依赖与密码套件评审](implementation/phase1-encrypted-source-vault-dependency-review.md)冻结精确 crypto / key-provider profile，[P1-S03a](implementation/phase1-source-vault-portable-crypto.md)又落地 portable cipher / wrap / AAD 与合成测试。[P1-S03b](implementation/phase1-source-vault-filesystem.md)已在独立 package 实现 filesystem envelope、不可覆盖发布与认证读取，已有 macOS 合成证据与 Windows ARM64 / NTFS 提升权限、普通用户及 ACL 验收证据，以及 Linux ARM64 / ext4 普通用户运行证据；[P1-S03c-2](implementation/phase1-source-vault-key-provider.md) 已实现独立 platform key provider 和合成错误 / 诊断验证，真实 OS 凭据、取消 / 锁定 / 重复项与清理尚未验收；P1-S04a 已实现仅供维护的密钥初始化 checkpoint，正文 migration 和 application integration 尚未实现；当前 SQLite v6 仍保存 inline plaintext body，独立 crypto package 不改变已有字节，也不授权使用真实个人资料。provider 默认测试不访问真实系统 store；上游 logger 可能输出 slot identity，真实调用前须限制敏感 logger target，第一方 Debug 脱敏不能替代该运行态验证。

P1-S03b 不新增持久化明文 staging；目录与对象替换、未知 envelope、错误 key、locator / metadata 不匹配均拒绝。失败残留保留供后续精确协调，不能从 `inspect_attempt` 推导 orphan 删除授权。私有 locator / attempt 仅供 adapter 持久化，不进入普通诊断；现有目录权限不会被该 adapter 自动修改，目录持久化失败不执行兼容降级。Windows 身份比较使用借用句柄的卷序号与完整 128 位文件 ID，保留引用防止比较期间 ID 重用；查询失败不回退时间戳，引用句柄不代替实际读写权限。现有证据不证明所有平台 / 文件系统的持久化、数据库提交或生产隐私验收。
