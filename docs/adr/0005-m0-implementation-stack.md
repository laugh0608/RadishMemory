...
把小文本正文暂存为 SQLite BLOB 是 M0 的实施选择，不是长期 Source Vault 格式。阶段 1 已通过 [ADR 0008](0008-phase1-encrypted-source-vault.md) 在 PDF、图片和大对象进入前冻结加密内容寻址对象存储、SQLite metadata 的事务协调和迁移边界，并由 [P1-S02](../implementation/phase1-encrypted-source-vault-dependency-review.md)冻结具体 cipher / key-provider dependency 选择；manifest / lockfile landing、adapter 与 migration 仍须独立评审和授权。
