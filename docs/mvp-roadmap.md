...
阶段 1 不把上述范围一次性展开为大批次。首个评审单元优先复用 M0 已成立的 Source Vault、全文召回、citation 与删除语义，只冻结用户显式选择的 UTF-8 文本 / Markdown 文件入口、来源身份 / 版本、幂等导入、精确导出、派生重建和删除边界；PDF / 图片解析、向量实现、模型 adapter 与 UI 分别在后续单元审查依赖、许可证、native build、隐私失败关闭和质量指标。
